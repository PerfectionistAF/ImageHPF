...
int main(int argc, char* argv[]) {
    ...
    int width, height, channels;
    int kernelSizeInt, sigmaInt;
    const char* inputImagePath;
    const char* outputImagePath;
    const char* kernelSize;
    const char* sigma;
    ...
    if (world_rank == 0) {
        ...
        img = stbi_load(inputImagePath, &width, &height, &channels, 0);
        if (img == NULL) {
    ...
    MPI_Bcast(&kernelSizeInt, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&sigmaInt, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (world_rank == 0) {
        ...
        stbi_write_jpg(outputImagePath, width, height, 1, resultImg, 100);
        std::cout << "Result image saved." << std::endl;
        stbi_image_free(img);
        delete[] resultImg;
    }
